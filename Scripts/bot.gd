...
@export var bottomRightLimit : Marker2D
...
func _physics_process(delta: float) -> void:
	if not is_on_floor():
		...
		if Input.is_action_just_pressed("jump") and BotMovement.read_input:
...
func inputs():
	dir = 0
	if Input.is_action_pressed("left") and BotMovement.read_input:
		dir -= 1
	if Input.is_action_pressed("right") and BotMovement.read_input:
		dir += 1
...
func cameraLimit():
	if topLeftLimit == null or bottomRightLimit == null:
		return
	
	var cam = get_node("Camera2D")
	cam.limit_left = topLeftLimit.position.x
	cam.limit_right = bottomRightLimit.position.x
	cam.limit_top = topLeftLimit.position.y
	cam.limit_bottom = bottomRightLimit.position.y
